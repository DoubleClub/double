...
import "./interfaces/dddx/IBaseV1Voter.sol";
import "./interfaces/dddx/IGauge.sol";
import "./interfaces/dddx/IBribe.sol";
import "./interfaces/dddx/IVotingEscrow.sol";
import "./interfaces/double/IFeeDistributor.sol";
import "./interfaces/double/IDoubleToken.sol";
import "./interfaces/double/ILpDepositToken.sol";
import "./interfaces/double/IVeDepositor.sol";
...
contract LpDepositor is Initializable, OwnableUpgradeable {
    ...
    // dddx contracts
    IERC20Upgradeable public DDDX;
    IVotingEscrow public votingEscrow;
    IBaseV1Voter public dddxVoter;

    // double contracts
    IDoubleToken public DOU;
    IVeDepositor public DDDXdou;
    ...
    // internal accounting to track DDDX fees for DDDXdou stakers and DOU lockers
    ...
    function initialize(
        IERC20Upgradeable _dddx,
        IVotingEscrow _votingEscrow,
        IBaseV1Voter _dddxVoter
    ) public initializer {
        __Ownable_init();
        ...
        dddxVoter = _dddxVoter;
    }


    function setAddresses(
        IDoubleToken _dou,
        IVeDepositor _dddxdou,
        address _doubleVoter,
        ...
        DDDX.approve(address(_dddxdou), type(uint256).max);
        _dddxdou.approve(address(_feeDistributor), type(uint256).max);
        votingEscrow.setApprovalForAll(_doubleVoter, true);
        votingEscrow.setApprovalForAll(address(_dddxdou), true);
    }

    /**
        @dev Ensure DDDX, DOU and DDDXdou are whitelisted
     */
    function whitelistProtocolTokens() external {
        require(tokenID != 0, "No initial NFT deposit");
        if (!dddxVoter.isWhitelisted(address(DDDX))) {
            dddxVoter.whitelist(address(DDDX), tokenID);
        }
        if (!dddxVoter.isWhitelisted(address(DDDXdou))) {
            dddxVoter.whitelist(address(DDDXdou), tokenID);
        }
        if (!dddxVoter.isWhitelisted(address(DOU))) {
            dddxVoter.whitelist(address(DOU), tokenID);
        }
    }

    /**
        @notice Get pending DDDX and DOU rewards earned by `account`
        @param account Account to query pending rewards for
        @param pools List of pool addresses to query rewards for
        @return pending Array of tuples of (DDDX rewards, DOU rewards) for each item in `pool`
     */
        ...
        returns (Amounts[] memory pending)
    {
        ...
        for (uint256 i = 0; i < pools.length; i++) {
            ...
            if (total > 0) {
                ...
                uint256 delta = IGauge(gaugeForPool[pool]).earned(address(DDDX), address(this));

                delta -= delta * 15 / 100;

                integral.dddx += 1e18 * delta / total;

                integral.dou += 1e18 * (delta * 10000 / 42069) / total;
            }


            Amounts storage integralFor = rewardIntegralFor[account][pool];
            if (integralFor.dddx < integral.dddx) {
                pending[i].dddx += balance * (integral.dddx - integralFor.dddx) / 1e18;
                pending[i].dou += balance * (integral.dou - integralFor.dou) / 1e18;
            }
        }
        return pending;
    }
    ...
    /**
        @notice Deposit Dddx LP tokens into a gauge via this contract
    ...
    function deposit(address pool, uint256 amount) external {
        require(tokenID != 0, "Must lock DDDX first");
        ...
        if (gauge == address(0)) {
            ...
            gauge = dddxVoter.gauges(pool);

            if (gauge == address(0)) {
                gauge = dddxVoter.createGauge(pool);
            }

            gaugeForPool[pool] = gauge;
            bribeForPool[pool] = dddxVoter.bribes(gauge);
    ...
    /**
        @notice Withdraw Dddx LP tokens
    ...
    /**
        @notice Claim DDDX and DOU rewards earned from depositing LP tokens
        @dev An additional 5% of DOU is also minted for `StakingRewards`
    ...
    function getReward(address[] calldata pools) external {
        ...
        for (uint256 i = 0; i < pools.length; i++) {
            ...
            claims.dddx += claimable[msg.sender][pool].dddx;
            claims.dou += claimable[msg.sender][pool].dou;
            delete claimable[msg.sender][pool];
        }

        if (claims.dddx > 0) {
            DDDX.transfer(msg.sender, claims.dddx);
            emit RewardPaid(msg.sender, address(DDDX), claims.dddx);
        }
        if (claims.dou > 0) {
            uint256 allMint = claims.dou * 1000 / 665;


            DOU.mint(msg.sender, claims.dou);
            emit RewardPaid(msg.sender, address(DOU), claims.dou);
            // mint an extra 5% for DDDXdou stakers

            DOU.mint(address(stakingRewards), allMint * 35 / 1000);
            emit RewardPaid(address(stakingRewards), address(DOU), allMint * 35 / 1000);


            DOU.mint(address(teamRewards),  allMint * 200 / 1000);
            emit RewardPaid(address(teamRewards), address(DOU), allMint * 200 / 1000);

            //Investors	10%
            DOU.mint(address(investorsRewards), allMint * 100 / 1000);
            emit RewardPaid(address(investorsRewards), address(DOU), allMint * 100 / 1000);
        }
    }
    ...
    /**
        ...
        @dev This method is unguarded, anyone can claim any reward at any time.
             Claimed tokens are streamed to DOU lockers starting at the beginning
    ...
    function claimLockerRewards(
        ...
        if (gaugeRewards.length > 0) {
            ...
            for (uint i = 0; i < gaugeRewards.length; i++) {
                ...
                require(reward != DDDX, "!DDDX as gauge reward");
        ...
        if (bribeRewards.length > 0) {
            ...
            uint256 dddxBalance = DDDX.balanceOf(address(this));
            ...
            for (uint i = 0; i < bribeRewards.length; i++) {
                ...
                if (reward == DDDX) {
                    // when DDDX is received as a bribe, add it to the balance
                    // that will be converted to DDDXdou prior to distribution
                    uint256 newBalance = DDDX.balanceOf(address(this));

                    unclaimedSolidBonus += newBalance - dddxBalance;
                    dddxBalance = newBalance;
                    continue;
                }
        ...
        if (amount > 0) {
            // lock 5% of earned DDDX and distribute DDDXdou to DOU lockers

            uint256 lockAmount = amount / 3;
            DDDXdou.depositTokens(lockAmount);

            IFeeDistributor(distributor).depositFee(address(DDDXdou), lockAmount);

            // distribute 10% of earned DDDX to DDDXdou stakers

            amount -= lockAmount;
            DDDX.transfer(address(stakingRewards), amount);
    ...
    function whitelist(address token) external returns (bool) {
        ...
        require(votingEscrow.balanceOfNFT(tokenID) > dddxVoter.listing_fee(), "Not enough veDDDX");

        dddxVoter.whitelist(token, tokenID);
        return true;
    }
    ...
    function onERC721Received(
        ...
        require(_operator == address(DDDXdou));
    ...
    function _updateIntegrals(
        ...
        if (total > 0) {
            ...
            uint256 delta = DDDX.balanceOf(address(this));
            address[] memory rewards = new address[](1);

            rewards[0] = address(DDDX);
            IGauge(gauge).getReward(address(this), rewards);

            delta = DDDX.balanceOf(address(this)) - delta;
            if (delta > 0) {
        ...
        if (user != address(0)) {
            ...
            if (integralFor.dddx < integral.dddx) {
                ...
                claims.dddx += balance * (integral.dddx - integralFor.dddx) / 1e18;
                claims.dou += balance * (integral.dou - integralFor.dou) / 1e18;
